...
from .fetcher import RedisFetcher
from . import job_runner

class InvalidJobError(Exception):
    pass

JOB_REQUIRED_KEYS = ['args', 'jid', 'job']

class Worker:
    def __init__(self, fetcher=None, runner=None):
        self.running = False
        if fetcher is None:
    ...
    def handle_failure(self, job, retries, error):
        logger.warning('could not process job %s: %s', job['job_id'], str(error))
        if retries > 0:
            logger.info('retrying to process %s (%d retries left)',
                        job['job_id'], retries)
            self.process_job(job, retries - 1)
        else:
            logger.error('job %s failed', job['job_id'])
